...
class matrix{
    ...
    matrix(){
      matrix(0);
		...
		template<typename U>
		matrix(matrix<M,N,U> const& m) {
			std::copy(m.data(), m.data()+M*N, data());
		}

		template<typename U>
		matrix& operator=(matrix<M,N,U> const& m){
			std::copy(m.data(),m.data()+M*N,data());
			return *this;
		}
...
template<unsigned int M, unsigned int N, typename T> inline
bool operator==(matrix<M,N,T> const& m1, matrix<M,N,T> const& m2){
